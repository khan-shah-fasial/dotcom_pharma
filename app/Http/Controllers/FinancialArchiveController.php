...
class FinancialArchiveController extends Controller
{
    ...
    {
        $archives = FinancialArchive::with('upload')->latest()->paginate(15);

        return view('backend.financial_archives.index', [
            'archives' => $archives,
            'types' => self::TYPES,
        ]);
    }
    ...
    {
        return view('backend.financial_archives.create', [
            'types' => self::TYPES,
        ]);
    }
    ...
    {
        $validated = $this->validateRequest($request, true, false);
        ...
        flash(translate('Financial archive created successfully.'))->success();

        return redirect()->route('financial-archives.index');
    }
    ...
    public function edit($id)
    {
        ...
        return view('backend.financial_archives.edit', [
            'archive' => $archive,
            'types' => self::TYPES,
        ]);
    }
    ...
    {
        $archive = FinancialArchive::findOrFail($id);
        $validated = $this->validateRequest($request, false, false);

        $uploadId = $archive->upload_id;
        ...
        $archive->update([
            'type' => $validated['type'],
            'upload_id' => $uploadId,
            'user_id' => $validated['user_id'] ?? $archive->user_id,
        ]);

        flash(translate('Financial archive updated successfully.'))->success();
