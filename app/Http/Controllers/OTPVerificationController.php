...
class OTPVerificationController extends Controller
{
    ...
    public function validateOtpCode(Request $request){
        ...
        if($user != null){
            ...
            // Prefer URL passed in request (hidden input in popup form) but skip login page itself
            if ($request->has('redirect_url') 
                && filter_var($request->redirect_url, FILTER_VALIDATE_URL) 
                && !str_contains($request->redirect_url, route('user.login'))
                && !str_contains($request->redirect_url, route('otp-verification-page'))) 
            {
    ...
    // public function send_delivery_status_bulk($order){
    //     SmsUtility::delivery_status_change_bulk($order);
    // }
