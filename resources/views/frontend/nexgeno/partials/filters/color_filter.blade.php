      ...
      @foreach (($colors ?? collect()) as $color)
        ...
        <label class="aiz-megabox mega-gap-between d-flex align-items-center justify-content-start pl-0 mr-2 text-center {{ $sel === $code ? 'selected' : '' }}">
          ...
          <div style="display: flex; gap: 10px;">
            ...
            <div class="fs-12 text-center text-dark bd-chked-clr color-name-fix" style="line-height:1; white-space: normal; word-break: break-word;">
              {{ $name }}
            </div>
          </div>
          {{-- Count below name (muted, only when >0) --}}
          <div class="fs-11 text-center text-muted">
            {{-- {{ $cnt > 0 ? '(' . $cnt . ')' : '' }} --}}
            {{  '(' . $cnt . ')' }}
          </div>
        </label>
