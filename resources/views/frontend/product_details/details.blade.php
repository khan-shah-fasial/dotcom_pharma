<div class="text-left">
    ...
    <hr>
    @if(is_user_loggedin())  <!--Display price to only loggedin user [by nexgeno]-->
    <!-- For auction product -->
                                ...
                                <div class="aiz-radio-inline">
                                    @foreach ($choice->values as $key => $value)
                                        <label class="aiz-megabox pl-0 mr-2 mb-0">
                                            ...
                                            <input type="radio" name="attribute_id_{{ $choice->attribute_id }}"
                                                value="{{ $value }}"
                                                @if ($key == 0 || get_user_subtype() == strtolower($value)) checked @endif> <!--added user_subtype role condition for role wise price based on session [by nexgeno]-->                                 
                                            <span
        ...
        <div class="mt-3" @if(!is_user_loggedin()) style="pointer-events:none;opacity:0.6" @endif>
            ...
            @if ($detailedProduct->digital == 0)
                ...
                @else
                    ...
                    <button type="button" class="btn btn-primary buy-now fw-600 add-to-cart min-w-150px rounded-0 border-radius-50"
                        @if (Auth::check() || get_Setting('guest_checkout_activation') == 1) onclick="addToCart()" @else onclick="showLoginModal()" @endif>
