                                                ...
                                                <tbody id="batch-rows-{{ $variantKey }}">
                                                    @if ($batches && $batches->count() > 0)
                                                        @foreach ($batches as $batchIndex => $batch)
                                                            @php
                                                                ...
                                                                $nonBatchRequest = data_get(request()->input('batches', []), $variantKey.'.'.$batchIndex.'.is_non_batch');
                                                                $isNonBatch = $nonBatchRequest !== null
                                                                    ? (int) $nonBatchRequest === 1
                                                                    : empty($batch->product_exp_date);
                                                                $batchCodeValue = data_get(request()->input('batches', []), $variantKey.'.'.$batchIndex.'.batch', $batch->batch);
                                                                if ($isNonBatch && trim((string) $batchCodeValue) === '') {
                                                                    $batchCodeValue = '-';
                                                                }
                                                            @endphp
                                                            <tr class="batch-row">
                                                                <td>
                                                                    ...
                                                                    <input type="text" name="batches[{{ $variantKey }}][{{ $batchIndex }}][batch]" class="form-control form-control-sm batch-code-input" value="{{ $batchCodeValue }}" placeholder="{{ translate('Batch code') }}" required>
                                                                </td>
                                                                <td class="text-center">
                                                                    <input type="hidden" name="batches[{{ $variantKey }}][{{ $batchIndex }}][is_non_batch]" value="0">
                                                                    <input type="checkbox" name="batches[{{ $variantKey }}][{{ $batchIndex }}][is_non_batch]" value="1" class="batch-non-batch" onchange="toggleNonBatchFields(this)" {{ $isNonBatch ? 'checked' : '' }}>
                                                                </td>
                                                                <td class="batch-mfg-cell">
                                                                    ...
                                                                    @endphp
                                                                    <input type="month" name="batches[{{ $variantKey }}][{{ $batchIndex }}][manufacturing_date]" value="{{ $isNonBatch ? '' : $batchMfgValue }}" class="form-control form-control-sm batch-mfg-input">
                                                                </td>
                                                                <td class="batch-expiry-cell">
                                                                    ...
                                                                    @endphp
                                                                    <input type="month" name="batches[{{ $variantKey }}][{{ $batchIndex }}][product_exp_date]" value="{{ $isNonBatch ? '' : $batchExpiryValue }}" class="form-control form-control-sm batch-expiry-input">
                                                                    @if(!$isNonBatch && function_exists('is_batch_expired') && is_batch_expired($batch))
                                                                ...
                                                                <td class="coa-uploader-cell batch-coa-cell">
                                                                    <div class="coa-uploader-wrapper" id="coa-wrapper-{{ $variantKey }}-{{ $batchIndex }}">
                                                                        <div class="input-group" data-toggle="aizuploader" data-type="document">
                                                                            ...
                                                                            <input type="hidden" name="batches[{{ $variantKey }}][{{ $batchIndex }}][coa]" class="selected-files batch-coa-input" value="{{ $isNonBatch ? '' : data_get(request()->input('batches', []), $variantKey.'.'.$batchIndex.'.coa', $batch->coa ?? '') }}">
                                                                        </div>
                                                                        <div class="file-preview box sm"></div>
                                                                    </div>
                                                                </td>
                                                    ...
                                                    @else
                                                        <tr class="batch-row">
                                                            <td>
                                                                @php
                                                                    $defaultIsNonBatch = (int) data_get(request()->input('batches', []), $variantKey.'.0.is_non_batch', 0) === 1;
                                                                    $defaultBatchCode = data_get(request()->input('batches', []), $variantKey.'.0.batch', '');
                                                                    if ($defaultIsNonBatch && trim((string) $defaultBatchCode) === '') {
                                                                        $defaultBatchCode = '-';
                                                                    }
                                                                @endphp
                                                                <input type="text" name="batches[{{ $variantKey }}][0][batch]" class="form-control form-control-sm batch-code-input" value="{{ $defaultBatchCode }}" placeholder="{{ translate('Batch code') }}" required>
                                                            </td>
                                                            <td class="text-center">
                                                                <input type="hidden" name="batches[{{ $variantKey }}][0][is_non_batch]" value="0">
                                                                <input type="checkbox" name="batches[{{ $variantKey }}][0][is_non_batch]" value="1" class="batch-non-batch" onchange="toggleNonBatchFields(this)" {{ $defaultIsNonBatch ? 'checked' : '' }}>
                                                            </td>
                                                            <td class="batch-mfg-cell">
                                                                <input type="month" name="batches[{{ $variantKey }}][0][manufacturing_date]" value="{{ $defaultIsNonBatch ? '' : data_get(request()->input('batches', []), $variantKey.'.0.manufacturing_date', '') }}" class="form-control form-control-sm batch-mfg-input">
                                                            </td>
                                                            <td class="batch-expiry-cell">
                                                                <input type="month" name="batches[{{ $variantKey }}][0][product_exp_date]" value="{{ $defaultIsNonBatch ? '' : data_get(request()->input('batches', []), $variantKey.'.0.product_exp_date', $stock && $stock->product_exp_date ? \Carbon\Carbon::parse($stock->product_exp_date)->format('Y-m') : '') }}" class="form-control form-control-sm batch-expiry-input">
                                                            </td>
                                                            <td>
                                                            ...
                                                            <td class="coa-uploader-cell batch-coa-cell">
                                                                <div class="coa-uploader-wrapper" id="coa-wrapper-{{ $variantKey }}-0">
                                                                    <div class="input-group" data-toggle="aizuploader" data-type="document">
                                                                        ...
                                                                        <input type="hidden" name="batches[{{ $variantKey }}][0][coa]" class="selected-files batch-coa-input" value="{{ $defaultIsNonBatch ? '' : ($stock->coa ?? '') }}">
                                                                    </div>
                                                                    <div class="file-preview box sm"></div>
                                                                </div>
                                                            </td>
            ...
            <tr class="batch-row">
                <td>
                    <input type="text"
                        ...
                        class="form-control form-control-sm batch-code-input"
                ...
                <td class="text-center">
                    ...
                    <input type="checkbox"
                        name="batches[` + variantKey + `][` + index + `][is_non_batch]"
                        value="1"
                        class="batch-non-batch"
                        onchange="toggleNonBatchFields(this)">
                </td>
                <td class="batch-mfg-cell">
                    <input type="month"
                        name="batches[` + variantKey + `][` + index + `][manufacturing_date]"
                        class="form-control form-control-sm batch-mfg-input">
                </td>
                <td class="batch-expiry-cell">
                    <input type="month"
                        name="batches[` + variantKey + `][` + index + `][product_exp_date]"
                        class="form-control form-control-sm batch-expiry-input">
                </td>
                <td>
