...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        <div class="col-md-6"><h1 class="h3">{{ translate('Edit Local Delivery Partner') }}</h1></div>
...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('local-delivery-partners.update', $localDeliveryPartner->id) }}" method="POST" id="local-delivery-partner-form">
            @csrf
            @method('PATCH')
            <div class="form-group row">
                <label class="col-md-2 col-form-label">{{ translate('Name') }} <span class="text-danger">*</span></label>
                <div class="col-md-9">
                    <input type="text" name="name" id="local-delivery-partner-name" class="form-control" value="{{ old('name', $localDeliveryPartner->name) }}" maxlength="255" required>
            ...
            <div class="form-group row">
                ...
                <div class="col-md-9">
                    <select name="status" class="form-control aiz-selectpicker">
                        <option value="active" @if(old('status', $localDeliveryPartner->status) == 'active') selected @endif>{{ translate('Active') }}</option>
                        <option value="inactive" @if(old('status', $localDeliveryPartner->status) == 'inactive') selected @endif>{{ translate('Inactive') }}</option>
                    </select>
                </div>
            </div>
